// ============================================================
// Wall Layout System — Builder / Input System
...
import type { Point2D, PointTuple, WallLayoutOptions } from "./types";
...
/**
 * Create a wall layout from an array of outer corner coordinates.
 *
...
export function fromPoints(
  ...
  if (points.length < 3) {
    throw new Error("A wall layout requires at least 3 corner points.");
  }
  const corners = points.map((p) => (Array.isArray(p) ? pt(p) : p));
  return new WallLayout(corners, options);
}
 ...
 *
 * The builder collects corner points and creates a WallLayout
 * when you call `.close()`.
 *
 * @example
 * ```ts
...
export class TraceBuilder {
  ...
  private options: WallLayoutOptions;
  ...
  /**
   * Close the loop back to the starting point and build the WallLayout.
   *
  ...
  close(): WallLayout {
    ...
    return new WallLayout(this.corners, this.options);
  }
}
...
/**
 * Create a rectangular wall layout.
 *
